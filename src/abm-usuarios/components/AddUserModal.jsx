...
function AddUserModal({ isOpen, onClose }) {
  ...
  useEffect(() => {
    if (isOpen) {
      loadRolesAndSeniorities();
    }
  }, [isOpen]);
  
  const loadRolesAndSeniorities = async () => {
    try {
      const rolesData = await rolesAPI.getAll();
      setRoles(rolesData);
      
      if (rolesData.length > 0 && !formData.rol) {
        setFormData(prev => ({ ...prev, rol: rolesData[0].rolId }));
      }
    } catch (error) {
  ...
  const validateStep1 = () => {
    ...
    if (!formData.nombreApellido.trim()) {
      newErrors.nombreApellido = "Este campo es requerido.";
    }

    if (!formData.usuarioAcceso.trim()) {
      newErrors.usuarioAcceso = "Este campo es requerido.";
    }

    if (!formData.correoUsuario.trim()) {
      newErrors.correoUsuario = "Este campo es requerido.";
    }
    ...
    if (errors.correoUsuario || errors.usuarioAcceso) {
      return false;
    }
  ...
  const hasErrorsInStep2 = () => {
    if (formData.correoEnvios.trim()) {
      return !formData.habilitado || errors.correoEnvios;
    }
    return false;
  };
    ...
    try {
      ...
      if (formData.imagenFirma) {
        try {
          ...
          signatureComplete = signatureComplete + (signatureComplete ? '|' : '') + imageBase64;
        } catch (error) {
      ...
      <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl p-8 z-40 w-[960px] max-h-[90vh] overflow-y-auto shadow-2xl">
        ...
        <div className="flex items-center justify-between mb-12 px-12">
          <div className="flex flex-col items-center gap-2 flex-1">
            ...
            <div className="text-center">
              <div className="text-sm font-normal">Información</div>
              <div className="text-sm font-normal">General</div>
            </div>
          </div>

          <div
            className={`flex-1 h-0.5 ${
              currentStep >= 2 ? "bg-gray-400" : "bg-gray-300"
            } mx-4 -mt-12`}
          ></div>

          <div className="flex flex-col items-center gap-2 flex-1">
            <div className="relative">
              <img src={mail} alt="Envío de Correos" className="w-16 h-16" />
            </div>
            <div className="text-center">
              <div className="text-sm font-normal">Envío de</div>
              <div className="text-sm font-normal">Correos</div>
            </div>
          </div>

          <div
            className={`flex-1 h-0.5 ${
              currentStep >= 3 ? "bg-gray-400" : "bg-gray-300"
            } mx-4 -mt-12`}
          ></div>

          <div className="flex flex-col items-center gap-2 flex-1">
            <div className="relative">
              <img src={pencil} alt="Firma del Usuario" className="w-16 h-16" />
            </div>
            <div className="text-center">
              <div className="text-sm font-normal">Firma</div>
              <div className="text-sm font-normal">del Usuario</div>
            </div>
          </div>
        </div>
              ...
              <div>
                <input
                  ...
                  className={`w-full px-4 py-3 rounded-lg bg-gray-100 outline-none ${
                    errors.nombreApellido ? "border-2 border-red-500" : ""
                  }`}
                />
                ...
                <div className="relative">
                  <select
                    ...
                    className={`w-full px-4 py-3 rounded-lg bg-gray-100 outline-none appearance-none cursor-pointer ${
                      errors.rol ? "border-2 border-red-500" : ""
                    }`}
                  >
                    {roles.map((rol) => (
          ...
          <div className="flex justify-center">
            <div className="space-y-6 max-w-[350px]">
              <div>
                ...
                <input
                  type="email"
                  placeholder="correo.envios@plan-in.com"
              ...
              <div className="bg-gray-50 pl-0 p-4 rounded-lg">
                ...
                <button 
                  ...
                  disabled={loading || validating.correoEnvios}
        ...
        <div className="flex gap-4 mt-8 justify-center">
          ...
          <button
            ...
            disabled={loading || (currentStep === 1 && hasErrorsInStep1()) || (currentStep === 2 && hasErrorsInStep2())}
            className={`flex-1 max-w-[180px] px-6 py-2 rounded-full text-white font-normal ${
              (loading || (currentStep === 1 && hasErrorsInStep1()) || (currentStep === 2 && hasErrorsInStep2()))
                ? 'bg-gray-400 cursor-not-allowed' 
                : 'bg-[#72BFDD] hover:bg-[#5fa8c4]'
            }`}
          >
            {loading ? 'Guardando...' : currentStep === 3 ? "Agregar Usuario" : "Continuar"}
          </button>
        </div>
      </div>
